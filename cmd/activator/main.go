...
func main() {
	...
	log.Printf("hellllllllllllllllllllllllllllllllllo") // LOG DURATION
	...
	logger.Infof("Stating Activator main took: %v", time.Since(start)) // LOG DURATION
	...
	startProxy := time.Now() // START TIMER

	logger.Debugf("MaxIdleProxyConns: %d, MaxIdleProxyConnsPerHost: %d, ProbeTimeout: %v, ProbeFrequency: %v",
	...
	logger.Infof("Activator->QP ET Enable TLS  main took: %v", time.Since(startProxy)) // LOG DURATION
	...
	logger.Infof("START THROLLER: %v", time.Since(start)) // LOG DURATION
	...
	logger.Infof("Connecting to Autoscale and concurrency report: %v", time.Since(start)) // LOG DURATION
	...
	logger.Infof("Activator Handler: %v", time.Since(start)) // LOG DURATION
	...
	logger.Infof("configMAPWATCHER: %v", time.Since(start)) // LOG DURATION
	...
	logger.Infof("verified if tlsenabled= https: %v", time.Since(start)) // LOG DURATION
	...
	logger.Info("Activator shutdown took: %v", time.Since(start)) // LOG DURATION
