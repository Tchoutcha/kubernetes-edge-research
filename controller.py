...
def next_poll_interval(rate: float | None, pending_count: int,
    ...
    Principe : viser un ratio constant d'environ un poll pour
    `requests_per_poll` vraies requêtes, quel que soit le débit réel.
    Concrètement : interval ≈ requests_per_poll / rate. Ce ratio reste le
    même que le trafic soit à 100 req/s ou à une requête toutes les 35s --
    aucun réglage par workload nécessaire, contrairement à un plafond fixe
    qui ne généralise qu'aux délais explicitement testés.

    - pending_count > 0 (un candidat de bascule s'accumule) -> retour
      immédiat au rythme de base, pour ne jamais retarder la détection
      d'un vrai changement de charge (garde-fou, prioritaire sur tout calcul).
    - rate is None (chauffe : pas encore de 2e lecture pour calculer un
      delta, ou reset après redémarrage/absence prolongée du provider) ->
      rythme de base, pour obtenir une vraie lecture au plus vite.
    - rate <= 0 (trafic confirmé nul, pas juste pas encore mesuré) ->
      rythme le plus lent (max_interval), rien à détecter dans l'immédiat.
    - rate > 0 -> interval proportionnel, borné entre base_interval (jamais
      plus rapide que le rythme de base) et max_interval (garde-fou pour
      ne jamais attendre indéfiniment à très faible débit).

    Fonction pure, testable sans policy ni provider réels.
    """
    if pending_count > 0:
...
def run_iterations(policy_cfg: PolicyConfig, cluster_cfg: ClusterConfig,
    ...
    # Applique explicitement le mode initial (détecté via les taints, ou
    # configuré) AVANT d'entrer dans la boucle -- ne pas se contenter de
    # savoir "quel mode on croit être", il faut aussi le FORCER. Sans ça,
    # un état déjà correct côté taint (ex: pas de taint = local1core) ne
    # garantit rien sur le pinning du scheduler, qui ne persiste pas tout
    # seul (process OS, pas un objet Kubernetes) -- ex: scheduler redémarré
    # entre deux runs du contrôleur, ou tout premier lancement.
...
def main():
    ...
    if args.check_config:
        ...
        print(f"poll_interval: {poll_interval}s -> max {poll_interval_max}s "
              f"(ratio ~1 poll / {poll_requests_per_poll} requêtes), "
              f"dry_run={dry_run}")
