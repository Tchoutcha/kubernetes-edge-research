...
MODEL_PATH = os.path.join(BASE_DIR, "dependencies", "model", "mobilenetv2-12.onnx")
LABELS_PATH = os.path.join(BASE_DIR, "dependencies", "data", "imagenet_classes.txt")


# ✅ CHARGEMENT DU MODÈLE UNE SEULE FOIS (IMPORTANT FaaS)
...
def run_model(img):
    # ✅ on réutilise le modèle déjà chargé
...
def handler(data: io.BytesIO = None):
    ...
    try:
        ...
        return json.dumps({
            "status": "success",
            "result": {"mobilenet": predicted_class},
