...
func (rt *revisionThrottler) acquireDest(ctx context.Context) (func(), *podTracker, bool) {
	...
	rt.logger.Infof("acquireDest took: %v", time.Since(start)) // LOG DURATION
...
func (rt *revisionThrottler) try(ctx context.Context, function func(dest string, isClusterIP bool) error) error {
	...
	rt.logger.Infof("Throttler Tryyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy  took") // LOG DURATION
	...
	for reenqueue {
		...
		if err := rt.breaker.Maybe(ctx, func() {
			...
			rt.logger.Infof(
				"Activator selected destination: rev=%s pod=%s clusterIP=%v",
				rt.revID.String(),
				tracker.dest,
				isClusterIP,
			)
			if tracker == nil {
				...
				rt.logger.Infof("Pas de pod dispo pour rev %s, réenqueue...", rt.revID.String())
			...
			rt.logger.Infof(
				"Activator request completed: rev=%s pod=%s err=%v",
				rt.revID.String(),
				tracker.dest,
				ret,
			)
	...
	rt.logger.Infof("Throttler Try took: %v", time.Since(start)) // LOG DURATION
	return ret
}
...
func (rt *revisionThrottler) handleUpdate(update revisionDestsUpdate) {
	...
	rt.logger.Infof("Activator update: clusterIP=%s pods=%d dests=%v", update.ClusterIPDest,
		len(update.Dests), update.Dests) // LOG UPDATE
		...
		for newDest := range update.Dests {
			...
			if !ok {
				if rt.containerConcurrency == 0 {
					...
					rt.logger.Infof(
						"Activator breaker created for %s maxConcurrency=%d",
						newDest,
						rt.containerConcurrency,
					) // LOG BREAKER CREATION
				} else {
					...
					rt.logger.Infof("Activator breaker created for %s maxConcurrency=%d", newDest, rt.containerConcurrency) // LOG BREAKER CREATION
				}
			}
			trackers = append(trackers, tracker)
			rt.logger.Infof("Activator pod discovered: %s", newDest) // LOG POD DISCOVERY
		}
...
func (t *Throttler) Try(ctx context.Context, revID types.NamespacedName, function func(dest string, isClusterIP bool) error) error {
	...
	t.logger.Infof("Activator received request for revision: %s/%s",
		revID.Namespace, revID.Name) // LOG REQUEST

	rt, err := t.getOrCreateRevisionThrottler(revID)

	t.logger.Infof("Creating throttler for revision %s/%s",
		revID.Namespace, revID.Name) // LOG THROTTLER CREATION
	if err != nil {
		return err
	}
...
func (t *Throttler) getOrCreateRevisionThrottler(revID types.NamespacedName) (*revisionThrottler, error) {
	...
	t.logger.Infof("getOrCreateRevisionThrottler took: %s", time.Since(start)) // LOG DURATION
	return revThrottler, nil
}
...
func (t *Throttler) revisionUpdated(obj interface{}) {
	...
	startREVISIONUPDATED := time.Now() // START TIMER
	...
	t.logger.Infof("Revision updated handled in %v", time.Since(startREVISIONUPDATED)) // END TIMER
}
...
func (t *Throttler) handleUpdate(update revisionDestsUpdate) {
	...
	starthandleupdate := time.Now() // START TIMER
	...
	t.logger.Infof("Revision update handled in %v", time.Since(starthandleupdate)) // END TIMER

}

func (t *Throttler) handlePubEpsUpdate(eps *corev1.Endpoints) {

	startHandle := time.Now() // START TIMER
	...
	t.logger.Infof("Public EPS update handled in %v", time.Since(startHandle)) // END TIMER
...
func (rt *revisionThrottler) handlePubEpsUpdate(eps *corev1.Endpoints, selfIP string) {
	...
	rt.logger.Infof("handlePubEpsUpdate handled in %v", time.Since(startHandle)) // END TIMER
}
