...
func (a *autoscaler) Scale(logger *zap.SugaredLogger, now time.Time) ScaleResult {
	...
	logger.Infof("For metric %s observed values: stable = %.1f; panic = %.1f; target = %.1f "+
		"Desired StablePodCount = %0.0f, PanicPodCount = %0.0f, ReadyEndpointCount = %d, MaxScaleUp = %0.0f, MaxScaleDown = %0.0f",
		metricName, observedStableValue, observedPanicValue, spec.TargetValue,
		dspc, dppc, originalReadyPodsCount, maxScaleUp, maxScaleDown)
	...
	logger.Infof("Autoscaler: stable=%f panic=%f target=%f pods=%d desired=%d excessBC=%f", observedStableValue,
		observedPanicValue, spec.TargetValue, readyPodsCount, desiredPodCount, excessBCF)

	duration := time.Since(start) //  END TIMER
	logger.Infof("⏱ Autoscaler decision: %v.", duration)
