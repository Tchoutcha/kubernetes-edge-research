...
func Main(opts ...Option) error {
	...
	logger.Infof("Starting queue-proxy : %v", start) // LOG DURATION
	logger.Info("Starting queue-proxy with start time:  %v", time.Since(start))
	...
	for name, server := range httpServers {
		go func(name string, s *http.Server) {
			...
			logger.Info("Starting  http server:  %v", time.Since(start))
