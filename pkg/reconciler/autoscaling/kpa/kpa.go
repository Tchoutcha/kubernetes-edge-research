...
func (c *Reconciler) ReconcileKind(ctx context.Context, pa *autoscalingv1alpha1.PodAutoscaler) pkgreconciler.Event {
	...
	//startSKS := time.Now() /// START TIMER de SKS RETRIEVAL

	sks, err := c.SKSLister.ServerlessServices(pa.Namespace).Get(sksName)

	//logger.Infof("SKS retrieval took -->>: %s", time.Since(startSKS)) // LOG DURATION
	...
	if sks == nil || sks.Status.PrivateServiceName == "" {
		...
		//startColdSKS := time.Now() /// START TIMER de SKS CREATION
		...
		//logger.Infof("Cold start (Reconcile SKS) took: %s", time.Since(startColdSKS)) // LOG DURATION
	...
	//startDecider := time.Now() /// START TIMER de DECIDER RECONCILIATION

	decider, err := c.reconcileDecider(ctx, pa)

	//logger.Infof("reconcileDecider took: %s", time.Since(startDecider))
	...
	//startScale := time.Now() /// START TIMER de SCALING OPERATION

	want, err := c.scaler.scale(ctx, pa, sks, decider.Status.DesiredScale)

	//logger.Infof("scaler.scale took: %s", time.Since(startScale)) // LOG DURATION
	...
	//startPods := time.Now() /// START TIMER de POD COUNTING
	...
	//logger.Infof("⏱ podCounter(compare the desired and observed resources to determine our situation): %v.", time.Since(startPods)) // LOG DURATION
	...
	//startActivators := time.Now() /// START TIMER de ACTIVATOR COUNTING

	numActivators := computeNumActivators(ready, decider)

	//logger.Infof("ComputeNumActivators took: %s", time.Since(startActivators)) // LOG DURATION
	...
	//startFinalSKS := time.Now() /// START TIMER de FINAL SKS RECONCILIATION

	sks, err = c.ReconcileSKS(ctx, pa, mode, numActivators)

	//logger.Infof("ComputeNumActivators took: %s", time.Since(startFinalSKS)) // LOG DURATION
	...
	//logger.Infof("status service name  took: %s", time.Since(startService))
	...
	logger.Infof(
		"KPA Scale decision for revision %s/%s: "+
			"currentPods=%d, calculatedWant=%d, desiredPods=%d, excessBurstCapacity=%d",
		decider.Namespace,
		decider.Name,
		ready,
		want,
		decider.Status.DesiredScale,
		decider.Status.ExcessBurstCapacity,
	)
	...
	logger.Infof("Cold start final  took: %s", time.Since(startInitial))
	return nil
}
...
func (c *Reconciler) computeStatus(ctx context.Context, pa *autoscalingv1alpha1.PodAutoscaler, pc podCounts, logger *zap.SugaredLogger) {
	...
	logger.Infof(
		"KPA Status for revision %s/%s after reconcile: currentPods=%d, desiredPods=%d, conditions=%v",
		pa.Namespace,
		pa.Name,
		pc.ready, // nombre de pods actuels
		pc.want,  // pods calculés par autoscaler
		conds,    // slice de strings lisible
	)
